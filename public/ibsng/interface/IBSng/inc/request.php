...
class Request { /* This is class for all requests
 ...
 */
    ...
    function __addAuthParams(&$params_arr) {
        ...
        $params_arr["auth_name"] = "system";
        $params_arr["auth_pass"] = "123456";
        $params_arr["auth_type"] = "ADMIN";
        $params_arr["auth_remoteaddr"] = IbsngConfig::getServerIPAddress();
    }
    ...
    function changeParam($key, $value) {/* change internal kept params array, set $key value to $value,
      useful when using one request object for multiple server calls
     */
        $this->params_arr[$key] = $value;
    }

    function send() {/* Warning: Deprecated, use sendAndRecv instead, just here for backward compatiblity
    ...
    function sendAndRecv() {/* send request, and return an Response instance.
      it will call $this->check(), this function should be override by children in case
      of they need some checking before sending request
     */
    ...
    function __check() {/* Children can override this function to check inputs before sending the request
...
class Response {
    ...
    function isSuccessful() {/* return True if request was successful

     */
        return $this->success;
    }

    function getResult() {/* return result of request. IF request was failed, return null
     */
        return $this->result;
    }

    function getError() {/* return error of a failed request. Null if request was successful
     */
        return $this->error;
    }
